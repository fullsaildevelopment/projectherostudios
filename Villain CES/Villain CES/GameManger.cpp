...
{	
	...
	tTimerInfo->globalTime = tTimerInfo->GetGlobalTime(tAugerTimers->tAppTimer, tTimerInfo->globalTime);
	tTimerInfo->localTime = tTimerInfo->GetGlobalTime(tAugerTimers->tSceneTimer, tTimerInfo->localTime);
...
int CGameMangerSystem::LoadMainMenu()
{
	...
	for (int nCurrentEntity = 0; nCurrentEntity < ENTITYCOUNT; nCurrentEntity++)
	{
		if (tThisWorld.atUIMask[nCurrentEntity].m_tnUIMask == COMPONENT_UIMASK | COMPONENT_LABEL | COMPONENT_BUTTON)
		{
			...
			{
				tTempPixelBuffer.hoverColor = XMFLOAT4(.65f, .65f, .65f, 1);
			}
			else
			{
